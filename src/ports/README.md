...
- **`postgres_repo.py`**: Implementa o `StateRepositoryProtocol` conectando-se a um banco de dados PostgreSQL. Utiliza um **Pool de Conexões Thread-Safe (`ThreadedConnectionPool` de 1 a 20 conexões)** com context manager dedicado (`_get_connection()`) para evitar saturação do DNS interno do Docker. Faz `upsert` das informações (pedidos e carrinhos), mantendo os estados atualizados (STG e STC) utilizando travas transacionais (`FOR UPDATE`) e instrumentado com spans do Sentry APM (`db.sql.query`).
